...
class Match {
    ...
    lookup(list) {
        return new Promise((resolve) => {
            this.db.collection('matches').find({
                'id': {
                    $in: list
                }
    ...
    multigrab(list, ip, io) {
        let joined = list.join('+').split('+0+')[0];
        return new api(this.db, ip, io).fetch(`/multi_match/all/matchids/${joined}`).then(
            (res) => {
                let processed = _.map(list, (n) => {
                    var temp = [
                    ...
                    if (temp[0].length !== 0 && temp[1].length !== 0) {
                        return this.processMatch(temp);
                    }
                });
                processed = _.compact(processed);
                this.db.collection('matches').insert(processed, {
                    multi: true
                });
                return processed;
            },
